...
retinex_only=combined_model.retinex_model
retinex_only.to(device)  # Move only this part to GPU
retinex_only.eval()  
...
for hazy_img, filename in test_data_dataloader:
    with torch.no_grad():
        ...
        lr_inv = retinex_only(hazy_img)
        J_ret = hazy_img * lr_inv
        
        # Clamp the images for saving
        J_ret = J_ret.clamp(0, 1)  # The final dehazed output
