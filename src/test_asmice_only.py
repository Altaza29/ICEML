...
model_path='./models/final_dehazing_model.pth'  
...
asm_only=combined_model.asm_model
asm_only.to(device)  # Move only this part to GPU
asm_only.eval()  
...
for hazy_img, filename in test_data_dataloader:
    with torch.no_grad():
        ...
        J_asm, t_map, _= asm_only(hazy_img)
        #J_asm, _, _ = asm_only(hazy_img)
        
        # Clamp the images for saving
        J_asm = J_asm.clamp(0, 1) #Final Dehazed Output
